...
app.get('/api/setup-i18n', async (req, res) => {
  try {
    const languages = [
      { code: 'zh-CN', name: '简体中文', flag: '🇨🇳', isDefault: true, sortOrder: 1, isActive: true },
      { code: 'en-US', name: 'English', flag: '🇺🇸', isDefault: false, sortOrder: 2, isActive: true },
      { code: 'es-ES', name: 'Español', flag: '🇪🇸', isDefault: false, sortOrder: 3, isActive: true },
    ];
    ...
    const translations = [
      ...
      { key: 'auth.welcome', namespace: 'auth', t: { 'zh-CN': '欢迎回来', 'en-US': 'Welcome back', 'es-ES': 'Bienvenido' } },
      // 首页
      ...
      { key: 'home.leaderboard', namespace: 'home', t: { 'zh-CN': '一周消费排行榜', 'en-US': '1 Week Spending Leaderboard', 'es-ES': 'Ranking semanal de gastos' } },
      { key: 'home.open_box', namespace: 'home', t: { 'zh-CN': '开箱', 'en-US': 'Open', 'es-ES': 'Abrir' } },
      { key: 'home.enter', namespace: 'home', t: { 'zh-CN': '进入', 'en-US': 'Enter', 'es-ES': 'Entrar' } },
      // 抽奖
    ...
    for (const item of translations) {
      await prisma.translation.upsert({
        where: { key: item.key },
        update: { translations: JSON.stringify(item.t) },
        create: { key: item.key, namespace: item.namespace, translations: JSON.stringify(item.t) },
      });
    }

    res.json({ success: true, message: '语言和词条已插入', languages: 3, translations: translations.length });
  } catch (e) {
