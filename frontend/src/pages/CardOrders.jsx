...
const STATUS_MAP = {
  PENDING: { text: '待处理', cls: 'bg-yellow-900/60 text-yellow-200', step: 1 },
  PROCESSING: { text: '处理中', cls: 'bg-blue-900/60 text-blue-200', step: 2 },
  SHIPPED: { text: '已发货', cls: 'bg-green-900/60 text-green-200', step: 3 },
  DONE: { text: '已完成', cls: 'bg-gray-700 text-gray-300', step: 4 },
  REJECTED: { text: '已拒绝', cls: 'bg-red-900/60 text-red-200', step: 0 },
};
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-bold text-orange-400">我的卡片订单</h2>
        <button
          onClick={onGoSubmit}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-gray-500">
          ...
          <button onClick={onGoSubmit} className="mt-4 text-orange-400 text-sm underline">去申请发货 →</button>
        </div>
      ) : (
        <div className="space-y-3">
          {orders.map((o) => {
            const items = parseItems(o.items);
            const st = STATUS_MAP[o.status] || { text: o.status, cls: 'bg-gray-700', step: 0 };
            return (
              <div
                key={o.id}
                onClick={() => setDetail(o)}
                className="bg-[#161616] border border-[#2a2a2a] rounded-xl p-4 cursor-pointer hover:border-orange-600/50 transition"
              >
                ...
                <div className="flex gap-2 mb-2">
                  {items.slice(0, 3).map((it, i) => (
                    <div key={i} className="w-12 h-16 bg-[#0d0d0d] rounded border border-[#2a2a2a] flex items-center justify-center text-xl">
                      🃏
                    </div>
                  ))}
                  {items.length > 3 && (
                    <div className="w-12 h-16 bg-[#0d0d0d] rounded border border-[#2a2a2a] flex items-center justify-center text-xs text-gray-500">
                      +{items.length - 3}
                    </div>
                  )}
                </div>
                <div className="flex justify-between items-center text-xs text-gray-500">
                  <div>共 {items.length} 张 · 收货人 {o.receiverName}</div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] p-4">
          <div className="bg-[#1a0f0c] border border-[#3d1a1a] rounded-2xl w-full max-w-sm max-h-[85vh] overflow-y-auto">
            <div className="flex justify-between items-center p-4 border-b border-[#2a1414]">
              <div className="text-sm font-bold text-orange-400">订单详情</div>
            ...
            <div className="p-4 space-y-4">
              ...
              {/* 卡牌明细 */}
              <div>
                <div className="text-xs text-gray-500 mb-2">卡牌明细</div>
              ...
              {/* 收货信息 */}
              <div>
                <div className="text-xs text-gray-500 mb-2">收货信息</div>
              ...
              {detail.trackingNo && (
                <div>
                  ...
                  <div className="bg-[#0d0d0d] rounded p-3 space-y-1 text-xs">
                    <div className="flex justify-between"><span className="text-gray-500">快递公司：</span><span>{detail.expressCompany || '-'}</span></div>
                    ...
                    <button
                      onClick={() => {
                        navigator.clipboard.writeText(detail.trackingNo);
                        alert('已复制快递单号');
                      }}
                      className="w-full mt-2 bg-[#2a1414] text-orange-400 text-xs py-1.5 rounded"
                    >
                      复制单号
                    </button>
                  </div>
              ...
              {detail.remark && (
                <div>
                  ...
                  <div className="bg-[#0d0d0d] rounded p-2 text-xs text-gray-300">{detail.remark}</div>
                </div>
              )}
              {detail.adminRemark && (
                <div>
                  <div className="text-xs text-gray-500 mb-1">处理备注</div>
                  <div className="bg-[#0d0d0d] rounded p-2 text-xs text-yellow-300">{detail.adminRemark}</div>
                </div>
              )}
            </div>
          </div>
...
function StatusTimeline({ status }) {
  ...
  if (status === 'REJECTED') {
    return (
      <div className="bg-red-900/30 border border-red-700/50 rounded-lg p-3 text-center text-red-300 text-xs">
        该申请已被拒绝
      </div>
    );
  }
      ...
      {steps.map((s, i) => {
        ...
        return (
          <div key={s.key} className="flex-1 flex flex-col items-center relative">
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${active ? 'bg-orange-600 text-white' : 'bg-[#2a1414] text-gray-500'}`}>
              {i + 1}
            </div>
            <div className={`text-[10px] mt-1 ${active ? 'text-orange-400' : 'text-gray-600'}`}>{s.label}</div>
            {i < steps.length - 1 && (
              <div className={`absolute top-3 left-1/2 w-full h-0.5 ${i < currentIdx ? 'bg-orange-600' : 'bg-[#2a1414]'}`}></div>
            )}
          </div>
        );
      })}
