...
export default function Orders() {
  ...
  return (
    <div className="p-4">
      <div className="text-center text-lg font-bold mb-6 text-orange-400">我的订单</div>
      {loading ? (
          ...
          {orders.map(o => (
            <div key={o.id} className="bg-[#1c0e0e] border border-[#3d1a1a] rounded-xl p-4 shadow-lg">
              <div className="flex justify-between items-start mb-2">
                <div className="text-xs text-gray-500">订单号: {o.id.slice(0, 12)}</div>
                <span className={`text-xs px-2 py-0.5 rounded ${o.status === 'PAID' ? 'bg-green-900 text-green-200' : 'bg-yellow-900 text-yellow-200'}`}>
                  {o.status === 'PAID' ? '已支付' : '待支付'}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <div>
                  <div className="text-white font-bold">{o.option.coins} + {o.option.bonus} 金币</div>
