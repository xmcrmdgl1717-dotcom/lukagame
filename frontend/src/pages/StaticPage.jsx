...
const FALLBACK_TITLES = { about: '关于我们', terms: '用户协议', privacy: '隐私政策', contact: '联系我们' };
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-gray-500">
          ...
          <div className="text-xs text-gray-600 mt-2">管理员可在后台「通知管理 → 文章管理」中配置</div>
        </div>
      ) : (
        <div>
          {article?.coverUrl && <img src={article.coverUrl} className="w-full rounded-xl mb-4" alt={title} />}
          <div className="prose prose-invert max-w-none text-sm text-gray-300 leading-relaxed article-content" dangerouslySetInnerHTML={{ __html: article?.content || '<p>暂无内容</p>' }} />
        </div>
      )}

      <style>{`
        .article-content h1, .article-content h2, .article-content h3 { color: #fff; font-weight: 700; margin-top: 1.2em; margin-bottom: 0.6em; }
        .article-content h2 { font-size: 1.15rem; }
        .article-content h3 { font-size: 1rem; }
        .article-content p { margin-bottom: 0.8em; color: #d1d5db; }
