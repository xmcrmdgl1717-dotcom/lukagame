...
export default function RechargeModal({ onClose }) {
  ...
  useEffect(() => {
    axios.get(`${API_URL}/api/recharge-options`).then(res => {
      setOptions(res.data);
      setLoading(false);
    });
  }, []);
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[300] p-4 backdrop-blur-sm">
      <div className="w-full max-w-sm bg-[#1a0f0c] border border-[#3d1a1a] rounded-2xl p-5 relative">
        ...
        <h2 className="text-center text-orange-400 font-bold mb-4">充值</h2>
          ...
          <div className="grid grid-cols-3 gap-3">
            {options.map(opt => (
              ...
              >
                ...
                {opt.bonus > 0 && <div className="text-[10px] text-green-400 mb-1">+{opt.bonus} 赠送</div>}
